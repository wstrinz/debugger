...
describe "Breakpoints" do
  ...
  describe "setting breakpoint in the current file" do
    ...
    def check_subject(field, value)
      debug_file("ex1") { subject.send(field).must_equal value }
    end

    it("must have correct pos") { check_subject(:pos, 10) }
    it("must have correct source") { check_subject(:source, fullpath("ex1")) }
    ...
    it("must return right response") do
      id = nil
      debug_file('ex1') { id = subject.id }
      check_output "Breakpoint #{id} file #{fullpath('ex1')}, line 10"
    end
  end
  ...
  describe "using shortcut for the command" do
    ...
    it "must set a breakpoint" do
      debug_file("ex1") { Debugger.breakpoints.size.must_equal 1 }
    end
  end
  ...
  describe "setting breakpoint to unexisted line" do
    ...
    it "must not create a breakpoint" do
      debug_file("ex1") { Debugger.breakpoints.must_be_empty }
    end

    it "must show an error" do
      debug_file("ex1")
      check_output "There are only #{LineCache.size(fullpath('ex1'))} lines in file \"ex1.rb\".", interface.error_queue
    end
  end
  ...
  describe "setting breakpoint to incorrect line" do
    ...
    it "must show an error" do
      debug_file("ex1")
      check_output 'Line 8 is not a stopping point in file "ex1.rb".', interface.error_queue
    end
  end
  ...
  describe "stopping at breakpoint" do
    ...
    it "must stop at the correct line" do
      debug_file("ex1") { state.line.must_equal 14 }
    end

    it "must stop at the correct file" do
      debug_file("ex1") { state.file.must_equal fullpath("ex1") }
    end

    it "must show a message" do
      debug_file("ex1")
      check_output "Breakpoint 1 at #{fullpath('ex1')}:14"
    end
  end
  ...
  describe "set breakpoint in a file" do
    describe "successfully" do
      before do
        enter "break #{fullpath('ex2')}:3", 'cont'
      end

      it "must stop at the correct line" do
        debug_file("ex1") { state.line.must_equal 3 }
      end

      it "must stop at the correct file" do
        debug_file("ex1") { state.file.must_equal fullpath("ex2") }
      end
    end
  ...
  describe "set breakpoint to a method" do
    describe "set breakpoint to an instance method" do
      ...
      it "must stop at the correct line" do
        debug_file("ex1") { state.line.must_equal 5 }
      end
    ...
    describe "set breakpoint to a class method" do
      ...
      it "must stop at the correct line" do
        debug_file("ex1") { state.line.must_equal 2 }
      end
  ...
  describe "disabling a breakpoint" do
    ...
    it "must have a breakpoint with #enabled? returning false" do
      debug_file("ex1") { Debugger.breakpoints.first.enabled?.must_equal false }
    end

    it "must not stop on the disabled breakpoint" do
      enter "cont"
      debug_file("ex1") { state.line.must_equal 15 }
    end
  end
